# cmds/violation_request.py
import json
import discord
from discord.ext import commands

from services.road_requests_service import (
    save_road_request,
    get_request_by_message_id,
    update_request_status_by_message_id,
    approve_request_create_camera,
)

with open("setting.json", "r", encoding="utf8") as jfile:
    jdata = json.load(jfile)

ADMIN_CHANNEL_ID = int(jdata["道路管理_channel"])
ENTRY_CHANNEL_ID = int(jdata["道路申請_channel"])
NOTIFY_CHANNEL_ID = int(jdata["通知_channel"])


# ========== Modal ==========
class ReportModal(discord.ui.Modal, title="回報違規路段"):
    camera = discord.ui.TextInput(label="監視器網址", style=discord.TextStyle.short, required=True)
    location = discord.ui.TextInput(label="地點", style=discord.TextStyle.short, required=True)
    desc = discord.ui.TextInput(label="補充說明", style=discord.TextStyle.paragraph, required=False)

    def __init__(self, bot, reporter: discord.User):
        super().__init__()
        self.bot = bot
        self.reporter = reporter

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message("✅ 已收到回報，管理員將審核。", ephemeral=True)

        embed = discord.Embed(title="🚨 新違規路段申請", color=discord.Color.red())
        embed.add_field(name="地點", value=self.location.value, inline=True)
        embed.add_field(name="監視器", value=self.camera.value, inline=True)
        embed.add_field(name="補充說明", value=self.desc.value or "無", inline=False)
        embed.set_footer(text=f"回報者: {interaction.user}")
        embed.timestamp = discord.utils.utcnow()

        # 發送到管理員頻道
        view = ManageView()  # persistent view
        admin_channel = self.bot.get_channel(ADMIN_CHANNEL_ID)
        if not admin_channel:
            return

        admin_msg = await admin_channel.send(embed=embed, view=view)

        # ✅ 存到 MySQL：road_requests（不再使用 reports.json）
        await save_road_request(
            guild_id=interaction.guild.id if interaction.guild else None,
            admin_channel_id=admin_channel.id,
            message_id=admin_msg.id,
            reporter_id=interaction.user.id,
            road_name=self.location.value,
            image_url=self.camera.value,  # 監視器網址
            note=self.desc.value,
            status="pending",
        )


# ========== 管理員操作 UI ==========
class ManageView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    async def _notify(self, interaction: discord.Interaction, result_text: str):
        req = await get_request_by_message_id(interaction.message.id)
        if not req:
            return

        reporter_id = req.get("reporter_id")
        notify_channel = interaction.client.get_channel(NOTIFY_CHANNEL_ID)

        if notify_channel and reporter_id:
            if result_text == "核准":
                await notify_channel.send(f"<@{reporter_id}> 你的申請已被 **{result_text}** ✅")
            elif result_text == "拒絕":
                await notify_channel.send(f"<@{reporter_id}> 你的申請已被 **{result_text}** ❌")
            else:
                await notify_channel.send(f"<@{reporter_id}> 你的申請需要 **{result_text}** ✏️")

    @discord.ui.button(label="Approve", style=discord.ButtonStyle.green, custom_id="roadreq_approve")
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content=f"✅ 已核准（由 {interaction.user} 操作）",
            view=None
        )

        # ✅ A：核准時立即建立/綁定 camera
        await approve_request_create_camera(
            message_id=interaction.message.id,
            reviewed_by=interaction.user.id,
        )

        await self._notify(interaction, "核准")

    @discord.ui.button(label="Reject", style=discord.ButtonStyle.red, custom_id="roadreq_reject")
    async def reject(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content=f"❌ 已拒絕（由 {interaction.user} 操作）",
            view=None
        )

        await update_request_status_by_message_id(
            message_id=interaction.message.id,
            status="rejected",
            reviewed_by=interaction.user.id,
        )
        await self._notify(interaction, "拒絕")

    @discord.ui.button(label="Request Edit", style=discord.ButtonStyle.gray, custom_id="roadreq_need_edit")
    async def request_edit(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content=f"✏️ 請回報者補充/修改（由 {interaction.user} 操作）",
            view=None
        )

        await update_request_status_by_message_id(
            message_id=interaction.message.id,
            status="need_edit",
            reviewed_by=interaction.user.id,
        )
        await self._notify(interaction, "補充/修改")


# ========== 使用者入口按鈕 ==========
class ApplyView(discord.ui.View):
    def __init__(self, bot):
        super().__init__(timeout=None)
        self.bot = bot

    @discord.ui.button(label="📋 填寫道路申請", style=discord.ButtonStyle.primary, custom_id="roadreq_apply")
    async def apply(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = ReportModal(self.bot, interaction.user)
        await interaction.response.send_modal(modal)


# ========== Cog ==========
class ViolationRequest(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name="report")
    @commands.has_permissions(administrator=True)
    async def report(self, ctx: commands.Context):
        if ctx.channel.id != ENTRY_CHANNEL_ID:
            await ctx.send("⚠️ 這個指令只能在指定的 #違規道路申請 頻道使用！", delete_after=5)
            return

        view = ApplyView(self.bot)
        embed = discord.Embed(
            title="🚦 道路違規申請入口",
            description="請點下方按鈕，填寫回報表單。",
            color=discord.Color.blue()
        )
        await ctx.send(embed=embed, view=view)


async def setup(bot: commands.Bot):
    await bot.add_cog(ViolationRequest(bot))