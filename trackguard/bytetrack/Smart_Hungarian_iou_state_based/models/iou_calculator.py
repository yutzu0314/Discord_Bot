...
class IoUCalculator:
    ...
    def __init__(self, config=None):
        ...
        # IoU calculation parameters - Tuned for better identity tracking
        self.iou_threshold = 0.3  # Minimum IoU untuk association (stricter)
        self.distance_threshold = 150.0  # Maximum distance untuk association (stricter)
        
        # Motion prediction parameters
        self.motion_weight = 0.7  # Weight untuk motion prediction
        self.max_motion_error = 80.0  # Maximum motion prediction error
    ...
    def calculate_combined_features(self, tracks: List, detections: List[Dict]) -> Dict[str, np.ndarray]:
        ...
        # TUNED: Stricter cost matrix untuk mengurangi ID switches
        cost_matrix = (
            1.0 - iou_matrix +                    # IoU cost (higher IoU = lower cost)
            0.3 * normalized_distance +           # Distance cost (increased weight)
            0.2 * normalized_motion               # Motion prediction cost (increased weight)
        )
