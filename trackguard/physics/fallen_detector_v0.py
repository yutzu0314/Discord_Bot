...
class FallenDetector(BaseDetector):
    ...
    def _compute_metric(self, track: object, velocity_field: 'VelocityField') -> float:
        ...
        # Jika AR < 0.9, motor sudah jatuh (horizontal) - SIMPLE DETECTION
        if current_ar < self.ar_fallen_max:
            score += 2.0  # Beri score tinggi untuk langsung trigger (>= 1.0 threshold)
            # Debug
            if hasattr(track, 'track_id') and track.track_id == 11:
                print(f"DEBUG: Motor jatuh terdeteksi! AR = {current_ar:.2f} < {self.ar_fallen_max}, score = {score}")
            # Tidak perlu check lainnya jika sudah jelas jatuh
            return score
    ...
    def _check_condition(self, metric_value: float, track: object) -> bool:
        ...
        SIMPLIFIED: Threshold metric >= 1.0 (minimal 1 dari 3 conditions)
        Untuk video pendek, langsung alarm jika ada tanda motor jatuh
        ...
        """
        # SIMPLIFIED: Minimal 1 dari 3 conditions cukup untuk video pendek
    ...
    def detect(self, tracks: List, velocity_field: 'VelocityField') -> List[Dict]:
        ...
        Flow:
        1. Detect fallen dari active tracks (YOLO deteksi)
